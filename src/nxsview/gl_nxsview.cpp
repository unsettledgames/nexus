...
void GLNxsview::paintEvent(QPaintEvent * /*event*/) {
    ...
    Matrix44f nfProj, nfView;
    Point3i viewport;

    trackball.GetView(nfProj, nfView);
	...
	for(unsigned int i = 0; i < scene.nodes.size(); i++) {
        ...
        // This actually SETS proj / view matrices to the frustum (doesn't set it to OpenGL though). getView wants OpenGL row-major matrices
        if(extracting)
            renderer.getView(nfProj.transpose().V(), (nfView * node.transform).transpose().V(), viewport.V());
