...
public:
    ...
    vcg::Color4b *colors(Signature &sig, uint32_t nvert, uint32_t version) {
        char *m = memory;
        m += nvert * sizeof(vcg::Point3f);
        if(sig.vertex.hasTextures())
            m += nvert * sizeof(vcg::Point2f);
        if (version <= 2 && sig.vertex.hasNormals())
            m += nvert * sizeof(vcg::Point3s);
        return (vcg::Color4b *)m;
    }
    vcg::Point3s *normals(Signature &sig, uint32_t nvert, uint32_t version) {
        char *m = memory + nvert * sizeof(vcg::Point3f);
        if(sig.vertex.hasTextures())
			m += nvert * sizeof(vcg::Point2f);
        if(version > 2 && sig.vertex.hasColors())
